...
public class Whence extends AppCompatActivity {
    ...
    private static final String TAG = "WhenceActivity";

    private MaterialEditText metWhenceAddress;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        llVariant = findViewById(R.id.whenceVariants);
        myCity = findViewById(R.id.whenceMyCity);
        myCity.setOnCheckedChangeListener((buttonView, isChecked) -> geoLocate());
        metWhenceAddress = findViewById(R.id.whenceAddress);
        // Слушатель ввода текста для поля адреса
        metWhenceAddress.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void afterTextChanged(final Editable s) {
                ...
                if (s.length() >= 3) {
                    ...
                    timer.schedule(new TimerTask() {
                        @Override
                        public void run() {
                            runOnUiThread(Whence.this::geoLocate);
                        }
    ...
    protected void onStart() {
        ...
        if (arguments != null) {
            // Получаем адрес пункта отправления
            whenceAddress = (Address) arguments.get("WhenceAddress");
        }
        // Если адрес пункта отправления не равен нулю
        if (whenceAddress != null) {
            // и переходим обратно к Ordering активити
            Intent intent = new Intent(Whence.this, Ordering.class);
    ...
    private void createVariantAddress(Address address) {
        ...
        linearLayout.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(Whence.this, Ordering.class);
            intent.putExtra("WhenceAddress", address);
            intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
            startActivity(intent);
            finish();
        });
    ...
    public void checkWhenceToMap(View view) {
        ...
        intent.putExtra("CheckPlace", "Whence");
        startActivity(intent);
    }
}
